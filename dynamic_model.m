...
for ep = 1:num_episodes % Episode
    ...
    for t = 1:num_trials
        ...
        % TODO: compute pr, the probability of choosing action A1, based on
        % the current value of engaged, a binary variable indicating if the
        % agent is in the engaged state or not
        % TODO: resample engaged
        if engaged == 1
        ...
        if engaged == 1
            engaged = 1 - (rand < lapse);
        else
            engaged = rand < recover;
        end
